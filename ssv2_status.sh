...
if [ "$http_ssv2" == "200" ]; then
	...
	if [ "$?" == "0" ]; then
		...
		# # 如果主开关打开就执行下面的数据
		ssv2_v2ray_enable=`dbus get v2ray_basic_enable`
...
v2ray_Core_update=`cat /etc/crontabs/root|grep v2ray_Core_update.sh`	
if [ -z "$v2ray_Core_update" ];then
	echo "#!/bin/sh" > /koolshare/scripts/v2ray_Core_update.sh
	echo "curl -X POST -d '{"id":9163122,"method":"v2ray_config.sh","params":[5],"fields":{"v2ray_basic_check_releases":"1"}}' http://127.0.0.1:8000/_api/" >> /koolshare/scripts/v2ray_Core_update.sh
	echo  "20 3 * * * /koolshare/scripts/v2ray_Core_update.sh" >> /etc/crontabs/root
fi